...
```python
# search-codegen/agent.py:94-117
...
```python
# search-codegen/run_experiment_1_3.py:213-244
...
**用例 1 证据摘录**（`evidence.json` 中的类型化输出，长串截断）：

```json
# wechat/evidence/ex13-asean-output-items.json
{
 "_excerpt": "实验 1-3 用例 1（ASEAN 首都距离）的 Responses API 类型化输出摘录（qwen3.8-flash，源 search-codegen/validation/runs/real_20260914T144331Z/evidence.json）",
 "output_item_types_in_order": [
  "reasoning",
  "message",
  "web_search_call",
  "reasoning",
 ...
 "web_search_call_1": {
  "id": "msg_9d3cde46-857e-49a8-abd6-…",
  "status": "completed",
  "action": {
   "query": "ASEAN member states capitals list 2026",
   "sources_head2": [
 ...
 "code_interpreter_call_1": {
  "status": "completed",
  "container_id": "msg_a574cb97-1a03-4589-bcce-cf57b95853bc",
  "code_head": "import itertools, math\n\n# Capitals of the 10 classic ASEAN member states.\n# Coordinates in decimal degrees (N/ …（截断）",
  "outputs_logs_head": "```\nTotal capital pairs enumerated (C(10,2)) = 45\n\nrank      km  capital A -> capital B\n   1   315.2  Kuala Lumpur (Malaysia)  <-> …（截断）",
  "note": "沙箱真实执行：45 对首都枚举与 haversine 计算发生在服务端容器里"
 }
}
```

这就是"服务端闭环"的直接证据：`web_search_call` 带着真实查询与来源 URL，`code_interpreter_call` 带着模型写的代码和**沙箱真实输出的日志**（"Total capital pairs enumerated (C(10,2)) = 45"）——不是模型声称算过，是容器里真的跑过。
...
**用例 2 证据摘录**——注意两个细节：① 第一轮**工具已声明**，但模型一轮都没调，"先澄清"是模型行为而非客户端扣着工具；② 续轮的 `previous_response_id` 与第一轮 `response_id` 精确相等：

```json
# wechat/evidence/ex13-clarification.json
{
 "_excerpt": "实验 1-3 用例 2（比特币技术分析）先澄清后续跑的证据摘录（同一 evidence.json）",
 "turn1_ambiguous_task": {
  "tools_declared": [
   {
    "type": "web_search",
  ...
  "note": "工具已声明，但模型一轮都没调用——先澄清是模型行为，不是客户端扣着工具",
  "response_id": "resp_c20e96b5-6912-969b-8cde-4f6049e5cac3"
 },
 "turn2_after_user_reply": {
  "request_previous_response_id": "resp_c20e96b5-6912-969b-8cde-4f6049e5cac3",
  "matches_turn1_response_id": true,
  "output_item_counts": {
   "reasoning": 8,
   "message": 5,
   "code_interpreter_call": 4,
   "web_search_call": 4
  },
  "note": "续轮用 previous_response_id 接续服务端会话；4 次搜索 + 4 次沙箱执行全部 completed"
 }
}
```
...
3. **验证要用独立参照，不能只看模型说了什么**——最近首都对要拿本地 haversine 再算一遍比对。
4. **自主性的真正价值在失败处理上**：沙箱没网时，模型的"假设 → 最小实验 → 得到证据 → 修正方法 → 透明披露"这一套动作，比它给出的任何数字都更能说明问题。
5. **分清失败的类型**：600 秒超时是**基础设施限制**，沙箱无网是**环境事实**，编造数字才是**行为问题**——混淆三类会导致完全错误的修复方向。
...
> **实用建议**：续轮是单轮计算量最重的类型；若提供商网关有流式超时限制，优先降思考档，而不是盲目重试。
