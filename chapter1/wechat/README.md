...
| 03 | [03-实验1-1-上下文消融.md](03-实验1-1-上下文消融.md) | 五组消融实验：去掉各组件后 Agent 如何失败；`Completed` 骗人；可依据性判定 | 19,999 | 2 | 12 段 |
| 04 | [04-实验1-2-Kimi原生搜索.md](04-实验1-2-Kimi原生搜索.md) | Kimi K3 + Formula 协议；RL 内化的是**决策**、执行仍在模型之外 | 19,998 | 1 | 7 段 |
| 05 | [05-实验1-3-搜索与代码执行.md](05-实验1-3-搜索与代码执行.md) | Responses API 逐步拆解；托管 web_search + code_interpreter；先澄清后执行；沙箱无网时的自主诊断 | 19,821 | 1 | 10 段 |
| 06 | [06-实验1-4-文生图工作流.md](06-实验1-4-文生图工作流.md) | 改写节点把指定文案丢进负面提示词；适配层被模型内化；系列小结 | 17,951 | 2 | 6 段 + 10 图 |

合计 **12 张 Mermaid 渲染图 + 10 张实验效果图**、**35 段注入代码/证据摘录**，全部篇目均在微信 20,000 字符正文上限之内。
...
03–06 三篇里的"证据上下文"块来自 `evidence/ex*.json`——它们由 `validation/*/evidence.json` **机器摘录**而成（只在长字符串处截断并标注），占位符不带行号、整文件注入，围栏语言按扩展名自动识别为 `json`。更新证据时重新摘录生成，不要手改。
...
06 篇的 10 张效果图来自 `image-gen-workflow/outputs/20260914T*/images/`（每张带 SHA-256），复制为 `images/exp14-*.png`。
