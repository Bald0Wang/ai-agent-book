...
![图 1](images/06-实验1-4-文生图工作流-fig1-61c47774.png)
...
```python
# image-gen-workflow/pipeline.py:27-39
...
```python
# image-gen-workflow/pipeline.py:110-149
...
```python
# image-gen-workflow/pipeline.py:230-291
...
```python
# image-gen-workflow/config.py:31-42
...
```python
# image-gen-workflow/main.py:89-134
...
```python
# image-gen-workflow/evidence.py:83-121
...
![图 2](images/06-实验1-4-文生图工作流-fig2-ad657df0.png)
...
看下面效果图里海报需求的对照会更直接：**原生路线把指定文案"深夜独处也清净"原样做成了海报大标题**——正是工作流路线丢掉的那句。这是"适配层被模型内化"最直观的一张实证图。
...
以下 10 张图全部来自真实运行产物（`image-gen-workflow/outputs/`，每张带 SHA-256）。每组上图为**工作流路线**（改写节点改写后生成），下图为**原生路线**（口语需求直接生成）。

### ① 耳机海报（具体需求）——本实验的核心实证

工作流路线。注意：画面上有一套模型自拟的英文文案（"SILENCE PRO X1"），但**指定的"深夜独处也清净"没有出现**——改写节点把它排除后，生图模型自己补了另一套文案：

![耳机海报·工作流路线](images/exp14-headphone-poster-workflow.png)

原生路线。**指定的文案被原样做成了海报大标题**——工作流路线丢掉的那句话，在这里一个字不差：

![耳机海报·原生路线](images/exp14-headphone-poster-native.png)

### ② 加班程序员（具体需求："周末加班，风格丧一点"）

工作流路线，"丧"被转译为视觉细节：

![加班程序员·工作流路线](images/exp14-programmer-overtime-workflow.png)

原生路线：

![加班程序员·原生路线](images/exp14-programmer-overtime-native.png)

### ③ 窗台绿植（具体需求："早晨的阳光刚好照进来"）

工作流路线：

![窗台绿植·工作流路线](images/exp14-windowsill-plant-workflow.png)

原生路线：

![窗台绿植·原生路线](images/exp14-windowsill-plant-native.png)

### ④ AGI 程序员（宽泛需求）——改写节点"编剧"面

工作流路线。改写节点补充的场景全部落地：瘫坐喝咖啡、监督全息屏、机器人在敲代码——需求里一个字都没提：

![AGI 程序员·工作流路线](images/exp14-agi-programmer-workflow.png)

原生路线：

![AGI 程序员·原生路线](images/exp14-agi-programmer-native.png)

### ⑤ 未来城市的早晨（宽泛需求）

工作流路线：

![未来城市·工作流路线](images/exp14-future-city-morning-workflow.png)

原生路线：

![未来城市·原生路线](images/exp14-future-city-morning-native.png)
